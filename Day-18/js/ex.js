...
b1Btn.addEventListener("click", function() {
    ...
    // Sửa lỗi: Kiểm tra biến n thay vì b
    if (isNaN(n) || n <= 0) {
    ...
    // Sửa lỗi: Chạy đến i <= n để in đủ n số
    for (let i = 3; i <= n; i++) {
        let next = a + b; // Số tiếp theo bằng tổng 2 số trước
        output += next + " ";
        
        // Cập nhật lại giá trị cho lần lặp sau
        a = b; 
        b = next;
    }

    result.innerText = output.trim(); // Dùng .trim() để xóa khoảng trắng thừa ở cuối
